...
def _row_uses_wildcard(row: dict[str, Any]) -> bool:
    if row.get("select_type") in ("*", None):
        return True
    return row.get("context") in ("*", None)


def _validate_generalization(row: dict[str, Any]) -> str | None:
    if not row.get("supported_in_production"):
        return None
    if not _row_uses_wildcard(row):
        return None
    key = row.get("semantic_schema_key", "?")
    paths = row.get("generalization_evidence_paths") or []
    min_fixtures = int(row.get("minimum_distinct_fixture_count", 2))
    if not row.get("generalization_verified"):
        return f"{key}: wildcard production requires generalization_verified=true"
    if len(paths) < min_fixtures:
        return f"{key}: insufficient generalization_evidence_paths ({len(paths)} < {min_fixtures})"
    if not row.get("scope_justification"):
        return f"{key}: wildcard production requires scope_justification"
    for rel in paths:
        if not (_REPO_ROOT / rel).is_file():
            return f"{key}: missing generalization evidence {rel}"
    return None
...
def _is_varlen_pattern(pattern: tuple[str | int, ...]) -> bool:
    return not pattern or pattern == ("VARLEN",) or list(pattern) == ["VARLEN"]
...
def _matches_option_type_pattern(
    ...
    *,
    ...
    if _is_varlen_pattern(template_pattern):
        # VARLEN with exact context/select_type: option types are not part of family identity.
        return True
    if actual is None:
        return False
...
def _matches_context(
    template_context: int | str | None,
    actual_context: int | str | None,
    *,
    allows_generalization: bool,
) -> bool:
    if template_context in ("*", None):
        return allows_generalization
    return template_context == actual_context


def _matches_select_type(
    template_type: int | str | None,
    actual_type: int | str | None,
    *,
    allows_generalization: bool,
) -> bool:
    if template_type in ("*", None):
        return allows_generalization
    return template_type == actual_type
...
def _matches_family(
    ...
    *,
    ...
    gen = template.allows_generalization
    if not _matches_select_type(template.select_type, select_type, allows_generalization=gen):
        return False
    if not _matches_context(template.context, context, allows_generalization=gen):
        return False
    if template.min_count != min_count or template.max_count != max_count:
        return False
    if template.selection_mode != mode:
        return False
    if not _matches_option_type_pattern(template.option_type_pattern, option_types, allows_generalization=gen):
        return False
    return True
...
def _specificity_score(
    ...
    *,
    ...
    if pat and not _is_varlen_pattern(pat):
        if option_types is not None and _matches_option_type_pattern(
            pat, option_types, allows_generalization=template.allows_generalization
        ):
            score += 4
    return score


def _allows_generalization(row: dict[str, Any]) -> bool:
    if not row.get("generalization_verified"):
        return False
    paths = row.get("generalization_evidence_paths") or []
    min_fixtures = int(row.get("minimum_distinct_fixture_count", 2))
    return len(paths) >= min_fixtures and bool(row.get("scope_justification"))
...
def _load_matrix() -> None:
    ...
    for row in rows:
        ...
        allows_gen = _allows_generalization(row)
        _REGISTRY[key] = SemanticResponseTemplate(
            ...
            allows_generalization=allows_gen,
        )
...
def get_template(
    ...
    *,
    ...
    if wildcard in _REGISTRY:
        tpl = _REGISTRY[wildcard]
        if tpl.allows_generalization and _matches_family(
            tpl,
            select_type=select_type,
            context=context,
            min_count=min_count,
